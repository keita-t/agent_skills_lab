...
def generate_pr_body(
    ...
    lines = [
        ...
        f"- Agent hosts: `{', '.join(agent_hosts)}`",
        f"- Action: `{action}`",
        "",
...
def _iter_common_manifest_owned_paths(
    ...
    for manifest in manifests:
        ...
        if remove_mode:
            relative_paths = sorted(
                manifest_common_relative_paths(manifest),
                key=lambda value: (value.count("/"), value),
                reverse=True,
            )
        else:
            relative_paths = sorted(manifest_common_relative_paths(manifest))
...
def _iter_adapter_payload_paths(
    manifests: list[EcosystemManifest],
    agent_hosts: list[str],
) -> list[tuple[EcosystemManifest, str, str]]:
    entries: list[tuple[EcosystemManifest, str, str]] = []
    seen_destinations: dict[str, str] = {}

    for manifest in manifests:
        for host in agent_hosts:
            for agent in manifest.agents:
                source_relative_path = ecosystem_agent_relative_path(manifest, agent)
                destination_relative_path = host_agent_destination_relative_path(host, agent)
                if destination_relative_path is None:
                    continue
                _record_adapter_destination(
                    seen_destinations,
                ...
                entries.append(
                    (manifest, source_relative_path, destination_relative_path)
                )

            for skill in manifest.skills:
                source_relative_path = ecosystem_skill_relative_path(manifest, skill)
                destination_relative_path = host_skill_destination_relative_path(host, skill)
                _record_adapter_destination(
                    seen_destinations,
                    destination_relative_path,
                    source_relative_path,
...
def _record_adapter_destination(
    seen_destinations: dict[str, str],
    destination_relative_path: str,
    source_relative_path: str,
    manifest: EcosystemManifest,
) -> None:
    owner = seen_destinations.get(destination_relative_path)
    if owner is None:
        seen_destinations[destination_relative_path] = manifest.slug
        return
    if owner == manifest.slug:
        return
    raise RuntimeError(
        "Host adapter path conflict between ecosystems "
        f"{owner} and {manifest.slug}: {destination_relative_path} "
        f"from {source_relative_path}."
    )
...
def execute_delivery_plan(
    ...
    resolved_agent_hosts = resolve_agent_hosts(agent_hosts, clone_path)
    ...
    if dry_run:
        if action == "install":
            ...
            changes = build_install_changeset(
                ...
                agent_hosts=resolved_agent_hosts,
            )
        else:
...
def build_install_changeset(
    ...
    resolved_agent_hosts = resolve_agent_hosts(agent_hosts, resolved_target_root)
    ...
    for _, relative_path in _iter_common_manifest_owned_paths(
        resolved_manifests,
        remove_mode=False,
    ):
    ...
    for _, source_relative_path, destination_relative_path in _iter_adapter_payload_paths(
        resolved_manifests,
        resolved_agent_hosts,
    ):
        source_path = resolved_source_root / source_relative_path
        if not source_path.exists():
            raise FileNotFoundError(f"Manifest-owned source path not found: {source_path}")
        destination = resolved_target_root / destination_relative_path
        changes.append(
            DeliveryChange(
                action="copy",
                relative_destination=destination_relative_path,
                destination=str(destination),
                source=str(source_path),
            )
        )
...
def _validate_delivery_changes(changes: list[DeliveryChange]) -> None:
    ...
    for change in changes:
        if change.relative_destination in PROTECTED_TARGET_INSTRUCTION_PATHS:
            raise RuntimeError(
                "Delivery changes must not modify target repository root instructions: "
                f"{change.relative_destination}"
            )
        destination = Path(change.destination)
