...
from ecosystem_lib import SUPPORTED_AGENT_HOSTS
...
def main() -> int:
    ...
    parser.add_argument(
        "--agent-host",
        action="append",
        choices=["auto", "all", *SUPPORTED_AGENT_HOSTS],
        help=(
            "AI tool host to install into. Repeat for multiple hosts. "
            "Defaults to auto-detection, then github-copilot when no host marker exists."
        ),
    )
    ...
    print(f"Agent hosts: {', '.join(result.agent_hosts)}")
