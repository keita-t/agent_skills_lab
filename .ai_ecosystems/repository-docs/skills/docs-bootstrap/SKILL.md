...
6. When importing this ecosystem into another repository, use the
   Ecosystem Delivery Orchestrator agent where the active host installed it,
   or another source-side delivery workflow that copies only the manifest-owned
   payload into the target repository.
