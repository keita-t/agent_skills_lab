...
| **`repository-docs`** | Manages repository docs: bootstraps the doc structure, syncs docs with code, refactors stale or awkward prose, and tracks TODO progress. |

### This repository uses its own docs ecosystem

agent_skills_lab is both the place where ecosystems are developed and a live target that runs `repository-docs` on itself. The documentation you see here is actively maintained by the same ecosystem you can install into your own repositories.

### Installing an ecosystem into another repository

Use the
[Ecosystem Delivery Orchestrator](.ai_ecosystems/repository-docs/agents/governance-ecosystem-delivery.agent.md)
...
for example, installing `repository-docs` also brings in `ecosystem-audit`.
You can choose the AI tool host to install for, such as Codex or Claude Code.
If you do not specify one, delivery detects the target repository settings and
uses GitHub Copilot when no host is found. Delivery does not modify target root
...
```bash
python .ai_ecosystems/deliver_ecosystem.py install --target-repo owner/repo --ecosystem repository-docs
```
...
| **`repository-docs`** | リポジトリの docs を管理します。ドキュメント構造のブートストラップ、コードとの同期、古い表現や読みにくい表現のリファクタリング、TODO 進捗の追跡を行います。 |

### このリポジトリ自身でも docs ecosystem を利用

agent_skills_lab は ecosystem を開発する場所であると同時に、`repository-docs` を自分自身に適用している live target でもあります。ここで見えているドキュメントは、あなたのリポジトリにもインストールできる同じ ecosystem によって維持されています。

### 別のリポジトリへのインストール

[Ecosystem Delivery Orchestrator](.ai_ecosystems/repository-docs/agents/governance-ecosystem-delivery.agent.md)
エージェントを使うと、インストール・削除のワークフローを対話形式で進められます。PR ベースのデリバリーフローを準備し、宣言済みの依存関係を自動で解決します（例：`repository-docs` をインストールすると `ecosystem-audit` も一緒に入ります）。

インストールは機能させたい AI ツールホスト（Codex や Claude Code など）を指定できます。
指定しない場合、インストール先のリポジトリの設定から配布先を自動検出します。
見つからない場合は GitHub Copilot を標準で使います。
