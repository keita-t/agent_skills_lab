...
def estimate_smart_markdown_tokens(
    selected_representations: dict[str, str],
    output_lang: str,
    index_depth: int,
) -> int:
    selected_paths = sorted(selected_representations)
    index_text = build_index(selected_paths, index_depth)
    codebase_text = "\n\n".join(
        selected_representations[path] for path in selected_paths
    )
...
def render_smart_codebase(
    ...
    if explicit_include:
        minimum_representations = {
            relative_path: stub_sections[relative_path]
            for relative_path in selected_paths
        }
        minimum_tokens = estimate_smart_markdown_tokens(
            minimum_representations,
            output_lang,
            index_depth,
        )
    ...
    if explicit_include:
        selected_representations = minimum_representations

    for relative_path in scored_paths:
        if relative_path in selected_representations:
            candidate_representations = dict(selected_representations)
            candidate_representations[relative_path] = full_sections[relative_path]
            candidate_tokens = estimate_smart_markdown_tokens(
                candidate_representations,
                output_lang,
                index_depth,
            )
            if candidate_tokens <= budget_tokens:
                selected_representations = candidate_representations
            continue

        full_candidate_representations = dict(selected_representations)
        full_candidate_representations[relative_path] = full_sections[relative_path]
        full_candidate_tokens = estimate_smart_markdown_tokens(
            full_candidate_representations,
            output_lang,
            index_depth,
        )
        if full_candidate_tokens <= budget_tokens:
            selected_representations = full_candidate_representations
            continue

        stub_candidate_representations = dict(selected_representations)
        stub_candidate_representations[relative_path] = stub_sections[relative_path]
        stub_candidate_tokens = estimate_smart_markdown_tokens(
            stub_candidate_representations,
            output_lang,
            index_depth,
        )
        if stub_candidate_tokens <= budget_tokens:
            selected_representations = stub_candidate_representations
            continue
