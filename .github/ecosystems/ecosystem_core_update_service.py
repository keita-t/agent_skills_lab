...
try:
    from mcp_models import UpdateCoreFilesInput, UpdateCoreFilesResult
except ModuleNotFoundError:
    @dataclass(frozen=True)
    class UpdateCoreFilesInput:
        target_repo: str = "."
        dry_run: bool = False

    @dataclass(frozen=True)
    class UpdateCoreFilesResult:
        target_repo: str
        dry_run: bool
...
def generate_core_file_payloads(repo_root: Path) -> dict[str, str]:
    manifests = load_ecosystem_manifests(repo_root)
    if not manifests:
        raise ValueError("No installed ecosystem manifests were found under .github/ecosystems/")
...
def apply_core_file_updates(request: UpdateCoreFilesInput) -> UpdateCoreFilesResult:
    repo_root = find_repo_root(Path(request.target_repo).resolve())
    manifests = load_ecosystem_manifests(repo_root)
    if not manifests:
        raise ValueError("No installed ecosystem manifests were found under .github/ecosystems/")

    generators = generate_core_file_payloads(repo_root)
    ...
    for rel_path in managed_files:
        if rel_path not in generators:
            ...
            warnings.append(f"Skipping unmanaged core file with no generator: {rel_path}")
            continue
        destination = repo_root / rel_path
        write_managed_markdown(destination, generators[rel_path], dry_run=request.dry_run)
        updated_files.append(rel_path)

    return UpdateCoreFilesResult(
