...
`.github/` 配下の ecosystem files は、manifest、routing、installer 挙動の
実装向け正本です。stable docs は、それらの内容を丸ごと複製せずに明快に説明します。
...
- Keep the split explicit between repository-level overview docs and
  implementation-facing `.github` ecosystem artifacts. /
  repository-level の概要文書と、実装向け `.github` ecosystem artifacts の分離を
  明示したまま保つ。
...
6. `.github/ECOSYSTEM_REGISTRY.md` and `.github/AGENT_SKILL_ROUTING.md` still
